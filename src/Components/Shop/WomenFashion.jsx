...
export  {Card}
class WomenFashion extends Component {
    constructor(props) {
        super(props)
        this.state = {
            articles:[]
    ...
    componentDidMount() {
        axios.get('/articles/Women').then(res => this.setState({
            articles:res.data
        }))
    ...
    render() {
        return (
            <section className="article-fashion">
                <h1 className="article-fashion-title">Women Fashion</h1>
                <div className="section-articles-show">
                   {this.state.articles.map(x=>{
                       return <Card addProduct={this.props.addProduct} articleName={x.name} price={x.price} image={x.image} />
                   })}
